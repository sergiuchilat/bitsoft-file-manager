...
export default class DbConfigValidator {
  ...
  private static validateDriver(driver: string) {
    // if (!Object.values(DbDriver).includes(driver as DbDriver)) {
    //   throw new Error(`Invalid db driver ${driver}`);
    // }
  }
  ...
  private static validateHost(host: string) {
    ...
    if (!isValidHostname(host)) {
      throw new Error(`Invalid db host: ${host}`);
    }
  }
